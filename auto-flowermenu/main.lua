...
if settings.labels == nil then settings.labels = {"Pick"} end
...
local function findLabel(caption)
  for index, label in ipairs(settings.labels) do
    if sameCaption(label, caption) then return index end
  end
  return nil
end
...
local function addLabel(caption)
  ...
  if caption == "" then return false end
  if findLabel(caption) then return false end
...
local function swapLabels(firstIndex, secondIndex)
  ...
  if labels[firstIndex] == nil or labels[secondIndex] == nil then return end
...
-- ---------------------------------------------------------------- picking
--
-- Per login, so tabbing to a character that never toggled it off still picks for that character. A
-- missing entry means "on": the addon's whole point is to act without being asked each time.
local pausedSessions = {}

hafen.client():options():keybindings():on("toggle", function()
  local session = hafen.session():current()
  if not session then return end
  pausedSessions[session] = not pausedSessions[session]
  hafen.log():write(pausedSessions[session] and "auto-flowermenu off" or "auto-flowermenu on")
end)
...
local function labelToPick(petals)
  for _, wanted in ipairs(settings.labels) do
    for _, petal in ipairs(petals) do
      if sameCaption(petal:label() or "", wanted) then return wanted end
    end
  end
  return nil
end
...
hafen.event():on("FlowerMenuAdded", function(petals, session)
  if pausedSessions[session] then return end
...
hafen.event():on("SessionRemoved", function(session) pausedSessions[session] = nil end)
...
refreshRows = function(listColumn)
  if not listColumn:exists() then return end     -- the page was left before the step came round

  for _, child in ipairs(listColumn:children():list()) do child:destroy() end
...
options:panel(function(root)
  ...
  local captionEntry = hafen.ui():entry():parent(addRow):size(ENTRY_WIDTH)
    :tooltip("a petal's caption, as the ring paints it: Pick, Chop, Harvest...")
  ...
  local function addTyped()
    ...
    hafen.timer():after(0, function()
      if not addLabel(caption) then return end
      if captionEntry:exists() then captionEntry:value("") end
      refreshRows(listColumn)
    end)
  end
